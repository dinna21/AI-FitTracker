...
const ActivityLog = () => {
  ...
  const handleQuickAdd = (activityName: string) => {
    setForm((p) => ({
      ...p,
      name:     activityName,
      calories: p.duration ? String(estimateCalories(activityName, Number(p.duration))) : "",
    }))
    setShowForm(true)
  }
  ...
  const handleDurationChange = (val: string) => {
    ...
    const estimated = form.name && dur > 0 ? String(estimateCalories(form.name, dur)) : ""
    setForm((p) => ({ ...p, duration: val, calories: estimated }))
  }

  const handleAdd = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.name.trim() || !form.duration || !form.calories) {
      toast.error("Please fill all fields"); return
    }
    setSubmitting(true)
    try {
      const { data } = await mockApi.activityLogs.create({
        data: { name: form.name, duration: Number(form.duration), calories: Number(form.calories) },
      })
          ...
          {todayLogs.length === 0 ? (
            <div className="flex flex-col items-center py-12 gap-3 text-center">
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500 max-w-[180px] leading-relaxed">
                Use Quick Add or add a custom activity above
              </p>
            </div>
          ) : (
